...
const glyphs: { t: string; s: React.CSSProperties; fs: number; d: number }[] = [
...
export default function Hero() {
	...
	useEffect(() => {
		...
		const accent2 = styles.getPropertyValue("--accent-2").trim() || "#6366f1";
		const dispose = initHeroScene(canvasRef.current, { accent, accent2 });
		return dispose;
	}, [resolvedTheme]);
	...
	return (
		<header className="hero" id="top">
			<div className="wrap hero-grid">
				<motion.div variants={container} initial={entr("hidden")} animate="show">
					<motion.div variants={item}>
						<span className="eyebrow">
							<span className="pulse"></span>Disponível para novos projetos
						</span>
					</motion.div>
				...
				<motion.div
					className="hero-3d"
					initial={entr({ opacity: 0, scale: 0.92 })}
				...
				>
					...
					>
						<span className="pulse"></span> {SITE.stats[0].num} projetos no ar
					</motion.div>
				</motion.div>
			</div>
		</header>
